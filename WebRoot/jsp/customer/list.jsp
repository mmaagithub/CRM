								...
								<TD>
									<TABLE id=grid
										...
										<TBODY>
											...
											<!-- 从request域中拿到pageBean -->
											<s:iterator value="#pageBean.list" var="cust">
												<!-- 遍历每次把遍历到的值放入ActionContext的键 -->
												<TR
											...
											<%-- <s:iterator value="#pageBean.list"><!-- 遍历每次把遍历到的值压入栈顶 -->
													<td><s:property value="cust_name"/></td><!-- 每次从栈顶中查找cust_name -->
												</s:iterator> --%>
											<%-- ${list}=${requestScope.list}从request域中找，
												相当于从request.getAttribute() 查找
												1.原生request  2.查找ValueStack的栈（Root）部分  3.查找ValueStack的Context部分（ActionContext）
												--%>
